resource "aws_instance" "mysql-worker" {
  ...
  provisioner "remote-exec" {
    inline      = [
              "sudo curl -sfL https://get.k3s.io | INSTALL_K3S_COMMIT=${var.k3s_version} INSTALL_K3S_EXEC=${var.worker_flags} sh -s -  --server https://${local.master_ip}:6443 --token ${local.node_token} --node-external-ip=${self.public_ip}"
    ]
  }
}
